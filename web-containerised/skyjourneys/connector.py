...
flightsUrl = os.path.join(SITE_ROOT, "mockFlights.json")
flightsReturnUrl = os.path.join(SITE_ROOT, "mockFlightsReturn.json")
achievementsUrl = "https://firebasestorage.googleapis.com/v0/b/journeys-6208e.appspot.com/o/achievements.json?alt=media&token=98db2fd9-95de-410d-ab31-99b3efcaebce"
...
def searchFlights(parameters):
    if parameters['returnReturn'] == 'checked':
        return json.load(open(flightsReturnUrl))

    return json.load(open(flightsUrl))
...
def registerUser(firstname, lastname, email, password):
    ...
    if isRegistered['status']:
        users.append({'id': users[-1]['id'] + 1, 'firstname': firstname,
...
def getAchievements():
    r = json.loads(requests.get(achievementsUrl).content)
